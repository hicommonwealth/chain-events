...
import { UniFactory } from './contractTypes/UniFactory';
...
export async function createApi(
  ...
  try {
    ...
    const uniContract = UniFactory.connect(contractAddresses.uni, provider);
...
export const subscribeEvents: SubscribeFunc<Api, RawEvent, ISubscribeOptions<Api>> = async (options) => {
  ...
  const pollMissedEventsFn = async () => {
    ...
    // defaulting to the uni contract provider, though could be any of the contracts
    const web3 = new Web3((api.uni.provider as Web3Provider)._web3Provider as WebsocketProvider);
