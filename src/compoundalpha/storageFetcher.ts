...
export class StorageFetcher extends IStorageFetcher<Api> {
  ...
  public async fetch(range?: IDisconnectedRange, fetchAllCompleted = false): Promise<CWEvent<IEventData>[]> {
    ...
    log.info(`Fetching Compoundalpha entities for range: ${range.startBlock}-${range.endBlock}.`);
    ...
    for (let i = 0; i < queueLength; i++) {
      ...
      log.debug(`Fetched Compoundalpha proposal ${proposal.id} from storage.`);
      ...
      if (proposalStartBlock >= range.startBlock && proposalStartBlock <= range.endBlock) {
        ...
        log.debug(`Compoundalpha proposal start block (${proposalStartBlock}) is before ${range.startBlock}, ending fetch.`);
        break;
      } else if (proposalStartBlock > range.endBlock) {
        // keep walking backwards until within range
        log.debug(`Compoundalpha proposal start block (${proposalStartBlock}) is after ${range.endBlock}, ending fetch.`);
      }
    }
    return results;
  }
