...
// TODO: Uncomment when contracts are ready
// import { ProtocolFactory } from './contractTypes/ProtocolFactory';
// import { ProjectFactory } from './contractTypes/ProjectFactory';
// import { CollectiveFactory } from './contractTypes/CollectiveFactory';
...
export async function createApi(
  ...
  try {
    ...
    // TODO: Uncomment when contracts are ready
    // const protocolContract = ProtocolFactory.connect(contractAddresses.protocol, provider);
    // const projectContract = ProjectFactory.connect(contractAddresses.project, provider);
    // const collectiveContract = CollectiveFactory.connect(contractAddresses.collective, provider);
    // await Promise.all([
    //   protocolContract.deployed(),
    //   projectContract.deployed(),
    //   collectiveContract.deployed(),
    // ]);
...
export const subscribeEvents: SubscribeFunc<Api, RawEvent, ISubscribeOptions<Api>> = async (options) => {
  ...
  const pollMissedEventsFn = async () => {
    ...
    // defaulting to the protocol contract provider, though could be any of the contracts
    // TODO: Uncomment when contracts are ready
    // const web3 = new Web3((api.protocol.provider as Web3Provider)._web3Provider as WebsocketProvider);
  ...
  try {
    log.info(`Subscribing to CWP contracts ${chain}...`);
